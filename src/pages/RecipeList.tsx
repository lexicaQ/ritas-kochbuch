      ...
      <div className="container mx-auto px-4 py-12">
        <div className="sticky top-24 z-30 -mt-8 bg-white rounded-2xl shadow-lg border border-cookbook-100 p-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-center">
            <div className="relative flex-1">
              ...
              <Input type="search" placeholder="Nach Rezepten suchen..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} onKeyDown={handleKeyPress} className="w-full pl-9" />
            </div>
            ...
            <div className="flex gap-2 flex-wrap md:flex-nowrap">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="flex items-center gap-2">
                    <Filter size={16} />
                    <span>Filter</span>
                    {totalFilters > 0 && <span className="ml-1 rounded-full bg-primary px-2 py-0.5 text-xs font-medium text-primary-foreground">
                        {totalFilters}
                      </span>}
                ...
                <DropdownMenuContent align="end" className="w-80 p-4 max-h-[80vh] overflow-auto border border-cookbook-200">
                  ...
                  <DropdownMenuGroup className="mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      {categories.map(category => <DropdownMenuCheckboxItem key={category} checked={selectedCategories.includes(category)} onCheckedChange={() => toggleCategory(category)} className="rounded-lg hover:bg-cookbook-50 border border-transparent hover:border-cookbook-100">
                          {category}
                  ...
                  <DropdownMenuGroup className="mb-4">
                    ...
                    <div className="flex gap-2 mt-2">
                      {difficulties.map(difficulty => <Button key={difficulty} variant="outline" size="sm" onClick={() => toggleDifficulty(difficulty)} className={cn("rounded-lg border", selectedDifficulties.includes(difficulty) ? "bg-cookbook-700 text-white border-cookbook-700" : "hover:bg-cookbook-50")}>
                          {difficulty}
                        </Button>)}
                    </div>
                  </DropdownMenuGroup>
                  ...
                  <DropdownMenuGroup className="mb-4">
                    ...
                    <div className="flex gap-2 mt-2 flex-wrap">
                      {timeRanges.map(time => <Button key={time} variant="outline" size="sm" onClick={() => setTimeRange(time)} className={cn("rounded-lg border flex items-center gap-1", selectedTime === time ? "bg-cookbook-700 text-white border-cookbook-700" : "hover:bg-cookbook-50")}>
                          <Clock size={12} />
                          {time}
                        </Button>)}
                    </div>
                  ...
                  <DropdownMenuGroup className="mb-4">
                    ...
                    <div className="flex gap-2 mt-2 flex-wrap">
                      {servingsOptions.map(serving => <Button key={serving} variant="outline" size="sm" onClick={() => setServings(serving)} className={cn("rounded-lg border flex items-center gap-1", selectedServings === serving ? "bg-cookbook-700 text-white border-cookbook-700" : "hover:bg-cookbook-50")}>
                          <User size={12} />
                          {serving}
                        </Button>)}
                    </div>
                  ...
                  <DropdownMenuGroup className="mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      {dietTypes.map(diet => <DropdownMenuCheckboxItem key={diet} checked={selectedDietType.includes(diet)} onCheckedChange={() => toggleDietType(diet)} className="rounded-lg hover:bg-cookbook-50 border border-transparent hover:border-cookbook-100">
                          {diet}
                        </DropdownMenuCheckboxItem>)}
                    </div>
                  </DropdownMenuGroup>
                  ...
                  <DropdownMenuGroup className="mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2 max-h-36 overflow-y-auto">
                      {commonIngredients.map(ingredient => <DropdownMenuCheckboxItem key={ingredient} checked={selectedIngredients.includes(ingredient)} onCheckedChange={() => toggleIngredient(ingredient)} className="rounded-lg hover:bg-cookbook-50 border border-transparent hover:border-cookbook-100">
                          {ingredient}
                        </DropdownMenuCheckboxItem>)}
                    </div>
                  </DropdownMenuGroup>
                  ...
                  <DropdownMenuGroup>
                    ...
                    <div className="flex gap-2 mt-2 flex-wrap max-h-36 overflow-y-auto">
                      {allTags.map(tag => <Button key={tag} variant="outline" size="sm" onClick={() => toggleTag(tag)} className={cn("rounded-lg border text-xs", selectedTags.includes(tag) ? "bg-cookbook-700 text-white border-cookbook-700" : "hover:bg-cookbook-50")}>
                          {tag}
                        </Button>)}
                    </div>
                  </DropdownMenuGroup>
              ...
              {(totalFilters > 0 || searchTerm) && <Button variant="ghost" onClick={resetFilters} className="flex items-center gap-1">
                  <X size={14} />
                  Zurücksetzen
                </Button>}
            </div>
          ...
          {totalFilters > 0 && <div className="mt-4 flex flex-wrap gap-2">
              {selectedCategories.map(category => <div key={category} className="flex items-center gap-1 rounded-full bg-cookbook-100 px-3 py-1 text-xs border border-cookbook-200">
                  <span>{category}</span>
                  <X size={14} onClick={() => toggleCategory(category)} className="cursor-pointer hover:text-cookbook-700" />
                </div>)}
              {selectedDifficulties.map(difficulty => <div key={difficulty} className="flex items-center gap-1 rounded-full bg-cookbook-100 px-3 py-1 text-xs border border-cookbook-200">
                  <ChefHat size={12} />
                  <span>{difficulty}</span>
                  <X size={14} onClick={() => toggleDifficulty(difficulty)} className="cursor-pointer hover:text-cookbook-700" />
                </div>)}
              {selectedTags.map(tag => <div key={tag} className="flex items-center gap-1 rounded-full bg-cookbook-100 px-3 py-1 text-xs border border-cookbook-200">
                  <span>{tag}</span>
                  <X size={14} onClick={() => toggleTag(tag)} className="cursor-pointer hover:text-cookbook-700" />
                </div>)}
              {selectedIngredients.map(ingredient => <div key={ingredient} className="flex items-center gap-1 rounded-full bg-cookbook-100 px-3 py-1 text-xs border border-cookbook-200">
                  <Utensils size={12} />
                  <span>{ingredient}</span>
                  <X size={14} onClick={() => toggleIngredient(ingredient)} className="cursor-pointer hover:text-cookbook-700" />
                </div>)}
              {selectedDietType.map(diet => <div key={diet} className="flex items-center gap-1 rounded-full bg-cookbook-100 px-3 py-1 text-xs border border-cookbook-200">
                  <Leaf size={12} />
                  <span>{diet}</span>
                  <X size={14} onClick={() => toggleDietType(diet)} className="cursor-pointer hover:text-cookbook-700" />
                </div>)}
              {selectedTime && <div className="flex items-center gap-1 rounded-full bg-cookbook-100 px-3 py-1 text-xs border border-cookbook-200">
                  <Clock size={12} />
                  <span>{selectedTime}</span>
                  <X size={14} onClick={() => setSelectedTime(null)} className="cursor-pointer hover:text-cookbook-700" />
                </div>}
              {selectedServings && <div className="flex items-center gap-1 rounded-full bg-cookbook-100 px-3 py-1 text-xs border border-cookbook-200">
                  <User size={12} />
        ...
        <div className="mt-8">
          {isLoading ? <div className="flex justify-center items-center py-20">
            ...
            </div> : filteredRecipes.length > 0 ? <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredRecipes.map((recipe, index) => <FadeIn key={recipe.id} delay={index * 0.05}>
